...
def fit_best_curve(x, y):
    ...
    try:
        ...
        if r_squared > 0:  # Only accept positive R²
            results['exponential'] = {
                'params': popt,
                'r_squared': r_squared,
...
# Plot data and trend lines for each condition (AXES SWAPPED)
for condition in conditions:
    data = df_combined[df_combined['condition'] == condition]
    # SWAP: x is now ISI_CV, y is now firing_rate
    ...
    if len(x_clean) > 3:
        ...
        if best_func is not None:
            ...
            try:
                ...
                print(f"\n{condition} (fallback to linear):")
                print(f"  R²: {r_value**2:.4f}")
                print(f"  Slope: {slope:.6f}")
    else:
        print(f"\n{condition}: Not enough data points for fitting")

# Customize the plot (AXES SWAPPED)
...
# Save the plot
plt.savefig('cv_fr_scatter_best_fit_swapped.png', dpi=300, bbox_inches='tight')
print(f"\nGraph saved as 'cv_fr_scatter_best_fit_swapped.png'")

# Show the plot
plt.show()

# Additional analysis: Compare best fits
print("\n" + "="*60)
print("BEST FIT MODEL COMPARISON")
print("="*60)

print("\nBest fitting models for each condition:")
for condition in conditions:
    if condition in best_fits:
        ...
        # Interpret parameters based on model type (AXES SWAPPED)
        if 'Linear' in fit_data['name']:
            slope, intercept = fit_data['params']
            print(f"  Equation: Firing_Rate = {slope:.6f} * ISI_CV + {intercept:.6f}")
            print(f"  Interpretation: Firing rate changes by {slope:.6f} Hz per unit increase in ISI CV")
...
# Rank models by R²
all_fits = [(condition, best_fits[condition]['name'], best_fits[condition]['r_squared'])
            for condition in best_fits.keys()]
all_fits.sort(key=lambda x: x[2], reverse=True)

print("\nRanked by R² (goodness of fit):")
for i, (condition, model, r2) in enumerate(all_fits, 1):
    print(f"{i}. {condition}: {model} (R² = {r2:.4f})")
